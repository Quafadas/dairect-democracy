...
case class Citizen[Alg[_[_, _, _, _, _]]](
    model: AssistantApi,
    ...
    service: Service[Alg],
    name: String,
    id: String // this is the assistant id.
)

// extension (runApi: RunApi)
//   def createThreadRunStream(
//       tool: FunctorAlgebra[Alg, IO],
//       assistantId: String,
//       thread: CreateThread
//   ) =
//     runApi.createThread(httpClient, assistantId, thread).flatMap { thread =>
//       runApi.getThread(httpClient, thread.id).flatMap { thread =>
//         fs2.Stream
//           .unfoldEval[IO, (ContinueFold, List[AiMessage]), List[AiMessage]]((ContinueFold.Continue, thread.messages)) {
//             (continue, allMessages) =>
//               continue match
//                 case ContinueFold.Stop => IO.pure(None)
//                 case ContinueFold.Continue =>
//                   runApi
//                     .runStep(httpClient, thread.id, allMessages)
//                     .flatMap { response =>
...
//                                 .traverse(fct =>
//                                   thread.toolkit.dispatcher.apply(fct.function).map { result =>
//                                     AiMessage.tool(
//                                       tool_call_id = fct.id,
...
// extension [Alg[_[_, _, _, _, _]]](citizen: Citizen[Alg])
