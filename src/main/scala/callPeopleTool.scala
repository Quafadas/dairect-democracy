...
object PersonTool extends IOApp.Simple:
  ...
  def run =
    clientR.both(key).use { (client, token) =>
      ...
      val prompt1 = "Find the person with id 00000000-0000-0000-0000-000000000000"
      ...
      val smithyDispatcher = testJiggy.openAiFunctionDispatch(peopleImpl) // SmithyOpenAi dispatcher
      val tools = testJiggy.toJsonSchema(peopleImpl) // SmithyOpenAi tool schema
      // -----------------------------
      println(tools)
